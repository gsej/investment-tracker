...
export class AccountSelectorComponent implements OnChanges, OnInit, OnDestroy {
  ...
  onAccountChanged(accountCode: string, event: Event) {
    ...
    if (this.multiSelectMode) {
      ...
      if (input.checked) {
        ...
        //   // If only one account is left selected, switch to radio button mode
        //   this.accountSelectorForm.controls['rad_selectedAccount'].setValue(Array.from(this.pendingSelectedAccounts)[0], { emitEvent: false });
        // }
      }
    } else {
      ...
      if (input.checked) {
       ...
       // this.accountSelectorForm.controls['rad_selectedAccount'].setValue(accountCode, { emitEvent: false });

        // update the checkbox controls
        // this.accounts.forEach(account => {
        //   if (account.accountCode === accountCode) {
        //     this.accountSelectorForm.controls["chk_" + account.accountCode].setValue(true);
        //   }
        //   else {
        //     this.accountSelectorForm.controls["chk_" + account.accountCode].setValue(false);
        //   }
    ...
    if (this.pendingSelectedAccounts.size === 1) {
      // In radio button mode, apply changes immediately
      this.selectedAccounts = new Set(this.pendingSelectedAccounts);
      this.accountChanged.next(Array.from(this.selectedAccounts));
    }

    console.log('onAccountChanged - After changes:', {
      pendingSelectedAccounts: JSON.stringify(Array.from(this.pendingSelectedAccounts)),
      selectedAccounts: JSON.stringify(Array.from(this.selectedAccounts))
    });
  }
  ...
  applyChanges() {
    ...
    console.log('applychanges - after update:', {
      pendingSelectedAccounts: JSON.stringify(Array.from(this.pendingSelectedAccounts)),
      selectedAccounts: JSON.stringify(Array.from(this.selectedAccounts))
    });
  }
